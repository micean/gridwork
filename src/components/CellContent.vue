...
const nonTopBorder = computed(() => {
  return selectedCellsStore.isCellSelected(props.path) && !selectedCellsStore.selectedBorder(props.path, 'top')
})
const nonBottomBorder = computed(() => {
  return selectedCellsStore.isCellSelected(props.path) && !selectedCellsStore.selectedBorder(props.path, 'bottom')
})
const nonLeftBorder = computed(() => {
  return selectedCellsStore.isCellSelected(props.path) && !selectedCellsStore.selectedBorder(props.path, 'left')
})
const nonRightBorder = computed(() => {
  return selectedCellsStore.isCellSelected(props.path) && !selectedCellsStore.selectedBorder(props.path, 'right')
})
...
const handleCellFocusByKey = (event: any) => {
...
const handlePressDelete = (event: any) => {
  if (event.path === props.path) {
    vars.value.content = '';
    model.value.text = '';
...
const handlePressInsert = (event: any) => {
  if (event.path === props.path) {
...
<template>
  <div
    ...
    <code
      ref="cellInput"
      :class="{'cell-input': true, 'editing': vars.editing}"
    ...
    >{{vars.content}}</code>
    <TableComponent v-if="model.innerGrid && model.innerGrid.length > 0" v-model="model.innerGrid" :parentPath="path"/>
  </div>
</template>
