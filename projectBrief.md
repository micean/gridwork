...
### 4. 表格渲染 (TableComponent.vue)
- 递归渲染嵌套表格
- 动态计算行列数
- 支持无限层级嵌套
- 响应式数据更新

### 5. 单元格交互 (CellContent.vue)
- **内容编辑**: 支持富文本编辑
- **样式控制**: 字体大小、背景颜色、边框样式
...
Keyboard Events → keys.ts → selectedCells Store
Mouse Events → selectedCells Store
Clipboard Events → clipboard.ts → selectedCells Store
```
